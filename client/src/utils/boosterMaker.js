  ...
  makePack() {
    ...
    function uncommonDeterminant() {
      ...
      let uncommonRunA = [
        ...
        "Vizier of the Scorpion", "Cruel Celebrant", "Jace's Triumph", "Prison Realm", "Mayhem Devil", "Cyclops Electromancer", "Ajani's Pridemate", "Deathsprout", "Dreadmalkin", "Rubblebelt Rioters", "Evolution Sage",
        "Bond of Passion", "Eternal Skylord", "Huatli's Raptor", "Bleeding Edge", "Tibalt's Rager", "Leyline Prowler", "Paradise Druid", "Ral's Outburst", "Bond of Discipline", "Vizier of the Scorpion", "Chandra's Triumph",
      ...
      function rareTest(x) {
        ...
        let raresPW = [
          "Ajani, the Greathearted", "Chandra, Fire Artisan", "Domri, Anarch of Bolas", "Jace, Wielder of Mysteries", "Karn, the Great Creator", "Nissa, Who Shakes the World",
          "Ral, Storm Conduit", "Sarkhan the Masterless", "Sorin, Vengeful Bloodlord", "Tamiyo, Collector of Tales", "Teferi, Time Raveler", "Ugin, the Ineffable", "Vivien, Champion of the Wilds", "Ajani, the Greathearted", "Chandra, Fire Artisan", "Domri, Anarch of Bolas", "Jace, Wielder of Mysteries", "Karn, the Great Creator", "Nissa, Who Shakes the World",
    ...
    const firstFour = uncommonDeterminant();
    for (let i=0; i<4; i++) {
      randomPack[i] = firstFour[i];
    }
    
    if (packSeed === 1) {
      
      let index = rng(66)-1;
      for (let i=4; i<6; i++) {											//Loop to determine commons from run A
        randomPack[i] = commonRunA[index];
        index = (index === commonRunA.length-1) ? 0 : index + 1;
      }
      
      index = rng(66)-1;
      for (let i=6; i<8; i++) {											//Loop to determine commons from run B
        randomPack[i] = commonRunB[index];
        index = (index === commonRunB.length-1) ? 0 : index + 1;
      }
      
      index = rng(55)-1;
      for (let i=8; i<14; i++) {											//Loop to determine commons from run C1
      ...
      let index = rng(66)-1;
      for (let i=4; i<7; i++) {											//Loop to determine commons from run A
        randomPack[i] = commonRunA[index];
        index = (index === commonRunA.length-1) ? 0 : index + 1;
      }
      
      index = rng(66)-1;
      for (let i=7; i<9; i++) {											//Loop to determine commons from run B
        randomPack[i] = commonRunB[index];
        index = (index === commonRunB.length-1) ? 0 : index + 1;
      }
      
      index = rng(55)-1;
      for (let i=9; i<14; i++) {											//Loop to determine commons from run C1
        randomPack[i] = commonRunC1[index];
        index = (index === commonRunC1.length-1) ? 0 : index + 1;
      }
      ...
      let index = rng(66)-1;
      for (let i=4; i<8; i++) {											//Loop to determine commons from run A
        randomPack[i] = commonRunA[index];
        index = (index === commonRunA.length-1) ? 0 : index + 1;
      }
      
      index = rng(66)-1;
      for (let i=8; i<10; i++) {											//Loop to determine commons from run B
        randomPack[i] = commonRunB[index];
        index = (index === commonRunB.length-1) ? 0 : index + 1;
      }
      
      index = rng(55)-1;
      for (let i=10; i<14; i++) {											//Loop to determine commons from run C2
      ...
      index = rng(66)-1;
      for (let i=8; i<11; i++) {											//Loop to determine commons from run B
        randomPack[i] = commonRunB[index];
        index = (index === commonRunB.length-1) ? 0 : index + 1;
      }
      
      index = rng(55)-1;
      for (let i=11; i<14; i++) {											//Loop to determine commons from run C2
        randomPack[i] = commonRunC2[index];
        index = (index === commonRunC2.length-1) ? 0 : index + 1;
      }
      ...
      index = rng(66)-1;
      for (let i=8; i<12; i++) {											//Loop to determine commons from run B
        randomPack[i] = commonRunB[index];
        index = (index === commonRunB.length-1) ? 0 : index + 1;
      }
      
      index = rng(55)-1;
      for (let i=12; i<14; i++) {											//Loop to determine commons from run C2
        randomPack[i] = commonRunC2[index];
        index = (index === commonRunC2.length-1) ? 0 : index + 1;
      }
    }
...
export default new boosterMaker();